...
CREATE TABLE bookLend (
	...
	PRIMARY KEY (b_code),
	FOREIGN KEY (b_code)references bookInfo (b_code)
	on update cascade
);
...
CREATE TABLE paymentIO (
	no          INTEGER NOT null AUTO_INCREMENT, -- 출납번호
	b_code      CHAR(4) NOT NULL, -- 도서코드
	m_code      CHAR(4) NOT NULL, -- 회원코드
...
grant select, insert, update, delete,trigger,show view
on book_project.*
to 'user_book'
...
insert into bookInfo(b_code,c_name,b_name,author,p_code,price,insert_date) values
...
INSERT into bookLend (b_code, is_lending, b_lend_count) VALUES
('T001', true, 3),
('T002', true, 1),
('T003', true, 4),
('T004', true, 1),
...
('M001', '김유정', '010-1234-1234',04524,'서울특별시 중구 세종대로 110',false, '1234', false),
('M002', '김태희', '010-1234-1234',35242,'대전광역시 서구 둔산로 100',false, '4567', false), 
('M003', '박보영', '010-1234-1234',41911,'대구광역시 중구 공평로 88',false, '5864', false),
('M004', '전지현', '010-1234-1234',47545,'부산광역시 연제구 중앙대로 1001',false, 'aaba', true),
('M005', 'Emma Watson', '010-1234-1234',41908,'대구광역시 중구 국채보상로139길 1',false, 'bbbb', false),
('M006', '고수', '010-1234-1234',41185,'대구광역시 동구 아양로 207',true, '8888', false), -- 탈퇴상태
('M007', '박보검', '010-1234-1234',41777,'대구광역시 서구 국채보상로 257',false, '9595', true),
('M008', '박형식', '010-1234-1234',42429,'대구광역시 남구 이천로 51',false, '5555', false),
('M009', 'Dan Stevens', '010-1234-1234',41590,'대구광역시 북구 옥산로 65',false, '4444', false),
('M010', '원빈', '010-1234-1234',42424,'대구 남구 중앙대로 220 3층',false, '8811', false);
...
INSERT INTO paymentIO (no, b_code, m_code, lend_date, return_date) values
...
-- 1. 반납필요 도서목록
select b.b_code,b_name,m.m_code,m_name,lend_date
...
select b.b_code,b_name,author,p.p_code,bl.b_lend_count,m.m_code,m.m_name,m.m_tel,lend_date
...
where i.b_code='T001' and m.m_code='M001';
 
-- 트랜잭션구분  3.출납 테이블에 입력된 반납일을 지정하여 저장!
UPDATE paymentIo
SET return_date='2017-03-06'
...
from bookInfo b join paymentIO i on b.b_code= i.b_code;
...
select b_code, b_name, p.publisher, author, price from book_info b 
left join publisher_info p
on b.p_code = p.p_code where b_name like '%자바%';


-- 2. 도서명으로 검색 시, 책의 이름이 중복 될 경우
-- 2-1. 중복확인하기 : 동일한 책의 이름이 있는지, 몇 권인지 확인
select count(*) from book_info where b_name like '%런던%';

-- 2-2. 동일한 책 이름이 중복되어 나오면, 새창에서 클릭시, 도서 코드로 재 검색
select b_code, b_name, p.publisher, author, price from book_info b left join publisher_info p
...
select count(*) from publisher_info where publisher like '%한빛%';
...
select p.m_code, m.m_name, lend_date, return_date, if(datediff(current_date, lend_date)>3, 'Y', 'N')
from member_info m join payment_io p on m.m_code=p.m_code where b_code = ?;
...
from payment_io p 
join member_info m on p.m_code = m.m_code
join book_info b on b.b_code = p.b_code where b.b_name like '%자바%';

-- 6-1. 동일한 책 이름이 중복되어 나오면, 새창에서 클릭시, 도서 코드로 재 검색
select m.m_code, m.m_name, lend_date, return_date, if(datediff(current_date, lend_date)>3, 'Y', 'N')
from payment_io p 
join member_info m on p.m_code = m.m_code
join book_info b on b.b_code = p.b_code where b.b_code=?


-- 7. 총 대여 횟수
select b_lend_count from b_lend_info where b_code=?;
